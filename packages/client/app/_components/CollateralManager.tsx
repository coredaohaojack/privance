  ...
  const loadCollateralData = useCallback(async () => {
    ...
    try {
      ...
      const [total, locked, available] = await Promise.all([
        getUserCollateral(address).catch(() => 0n),
        getTotalLockedCollateral(address).catch(() => 0n),
        getAvailableCollateral(address).catch(() => 0n),
      ]);

      // Handle undefined or null values - default to 0
      const totalBigInt = total ?? 0n;
      const lockedBigInt = locked ?? 0n;
      const availableBigInt = available ?? 0n;

      setCollateralBalance(ethers.formatEther(totalBigInt));
      setLockedCollateral(ethers.formatEther(lockedBigInt));
      setAvailableCollateral(ethers.formatEther(availableBigInt));
    } catch (err: any) {
      console.error("Error loading collateral data:", err);
      // Don't show error for new users with no collateral
