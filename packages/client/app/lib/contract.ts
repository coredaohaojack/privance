...
// Contract addresses - with fallback to hardcoded values for Sepolia
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTRACT_ADDRESS || "0x4913b4eb032eB8f8D76bFc542dB48038115890a4";
const COLLATERAL_MANAGER_ADDRESS = process.env.NEXT_PUBLIC_COLLATERAL_MANAGER || "0xA665238920d557533A7a925E8598F50F547584C8";
const REPAYMENT_TRACKER_ADDRESS = process.env.NEXT_PUBLIC_REPAYMENT_TRACKER || "0x6d8383DA640306f5cD649B6ECBec1777024e3230";
const SEPOLIA_CHAIN_ID = process.env.NEXT_PUBLIC_CHAIN_ID || "11155111";

// Log for debugging
if (typeof window !== "undefined") {
  console.log("[contract.ts] CONTRACT_ADDRESS:", CONTRACT_ADDRESS);
  console.log("[contract.ts] COLLATERAL_MANAGER_ADDRESS:", COLLATERAL_MANAGER_ADDRESS);
  console.log("[contract.ts] SEPOLIA_CHAIN_ID:", SEPOLIA_CHAIN_ID);
}
...
export const encryptData = async (fhevmInstance: FhevmInstance, value: bigint, signerAddress: string) => {
  ...
  if (!CONTRACT_ADDRESS || !CONTRACT_ADDRESS.startsWith("0x") || CONTRACT_ADDRESS.length !== 42) {
    console.error("[encryptData] Invalid CONTRACT_ADDRESS:", CONTRACT_ADDRESS);
    throw new Error(`Contract address is not valid: ${CONTRACT_ADDRESS}`);
  }

  // Validate signer address
  if (!signerAddress || !signerAddress.startsWith("0x") || signerAddress.length !== 42) {
    console.error("[encryptData] Invalid signerAddress:", signerAddress);
    throw new Error(`Signer address is not valid: ${signerAddress}`);
  }
  ...
  console.log("[encryptData] Creating encrypted input for contract:", CONTRACT_ADDRESS);
  console.log("[encryptData] Signer address:", signerAddress);
  console.log("[encryptData] Value:", value.toString());
